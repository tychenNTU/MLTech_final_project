...
# load data
train_data_raw = pd.read_csv("./reg_train_data.csv")
val_data_raw = pd.read_csv("./reg_val_data.csv")
with open("reg_added_features", "rb") as fd:
...
y_train = np.array(train_data_raw["adr"])
X_val = val_data_raw[features_spec]
y_val = np.array(val_data_raw["adr"])
...
model.compile(loss="mse", optimizer=keras.optimizers.SGD(lr=1e-4), metrics=["mse"])
log_path = "./reg_log/" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
os.mkdir(log_path)
...
shutil.copy("./data_for_reg.py", data_code_path)
shutil.copy("./train_reg.py", train_code_path)
history = model.fit(X_train, y_train, validation_data=(X_val, y_val),
                    epochs=1, callbacks=[keras.callbacks.EarlyStopping(patience=100, restore_best_weights=True)])
